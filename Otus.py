...
class Otus:
    ...
    def masu_status(self):
        if self.masu == 1 or self.masu == 0:
            print(bcolors.FAIL + bcolors.BOLD +
                  "Otuksella on näläkä. RUOKI >:(" + bcolors.ENDC)
        elif self.masu < 0:
            self.elossa = False
            print("Otuksesi kuoli nälkään.")
            print(bcolors.BOLD + "Hävisit pelin." + bcolors.ENDC)
        elif self.masu == 2:
            print(bcolors.WARNING + "Otuksellasi on hyvät oltavat :)" +
                  bcolors.ENDC)
            ...
            print(bcolors.OKGREEN + bcolors.BOLD +
                  "Otusta ei voi ruokkia enempää." + bcolors.ENDC)
    ...
    def vibat_status(self):
        if (self.vibat == 1 or self.vibat == 0) and self.elossa:
            print(bcolors.FAIL + bcolors.BOLD +
                  "Otuksella on ihan kamala olo. LEIKI TAI KÄY HUONOSTI >:(" + bcolors.ENDC)
        elif self.vibat < 0:
            ...
            print(bcolors.OKGREEN + bcolors.BOLD +
                  "Otuksen viba ei voi olla tämän parempi." + bcolors.ENDC)
